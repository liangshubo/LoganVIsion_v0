...
class Trainer_2loss(Trainer_1model_2loss):
    ...
    def cala_loss(self, output, mask,gt):
        loss1 = self.loss(output[0], mask)
        loss2 = self.loss2(output[1], gt)
        loss = loss1 + loss2
    ...
    def model_backward(self,batch,image, ground_truth,mask):
        self.optimizer_zero_grad()
        output = self.model_forward(image)
        #output = self.output_process(output)
        loss,loss1,loss2 = self.cala_loss(output, ground_truth,mask)
        loss.backward()
    ...
    def train(self):
        ...
        for batch, (image, ground_truth,mask) in enumerate(self.loader_train):
            # - - - - - - - - - - - data_process - - - - - - - - - -
            image, ground_truth,mask = self.prepare(image, ground_truth,mask)
            image, ground_truth,mask = self.down_sample(image, ground_truth,mask)
            mask = mask[0]
            # - - - - - - - - - - - start time - - - - - - - - - - -
            timer_data.hold()
            timer_model.tic()
            # - - - - - - - - - - -model forward and loss_optimizer backward
            self.model_backward(batch,image,ground_truth,mask)
            timer_model.hold()
    ...
    def test(self):
        ...
        for idx_data, d in enumerate(self.loader_test):
            ...
            for image, ground_truth,mask, nameext in tqdm(d, ncols=80):
                start = time.time()
                image, ground_truth,mask = self.prepare(image, ground_truth,mask)
    ...
    def eval(self):
        ...
        for idx_data, d in enumerate(self.loader_test):
            ...
            for image, ground_true,mask, nameext in tqdm(d, ncols=80):
                image, ground_true,mask = self.prepare(image, ground_true,mask)
                image_d ,ground_true,mask = self.down_sample(image,ground_true,mask)
                t_start = time.time()

...
class Trainer(Trainer_1model_1loss):
    ...
    def cala_loss(self, output,ground_true, mask):
        loss1 = self.loss(output*mask,ground_true*mask)
        loss2 = self.loss(output*(1-mask),ground_true*(1-mask))
        loss = loss1 *10 + 1*loss2
        return loss
    ...
    def model_backward(self,batch,image, ground_truth,mask):
        ...
        loss = self.cala_loss(output, ground_truth,mask)
        loss.backward()
    ...
    def train(self):
        ...
        for batch, (image, ground_truth,mask) in enumerate(self.loader_train):
            ...
            self.model_backward(batch,image,ground_truth,mask[0])

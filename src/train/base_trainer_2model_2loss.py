...
class Trainer_2model_2loss(BaseTrainer):
    def __init__(self,args,loader,model_list,loss_list,ckp):
        super(Trainer_2model_2loss,self).__init__(args,loader,model_list,loss_list,ckp)
    ...
    def get_epoch(self):
        ...
        epoch = self.optimizer2.get_last_epoch() + 1
        return epoch

    def get_lr(self):
        lr = self.optimizer.get_lr()
        lr2 = self.optimizer2.get_lr()
        return lr,lr2
    ...
    def optimizer_zero_grad(self):
        """
        default the two optimizer will zero_gard() together
        if not ,you can rewrite them and used in self.model_backward
        Returns:
    ...
    def trainepoch_ckp_loss_logstart(self):
        ...
        epoch = self.get_epoch()
        lr,lr2 = self.get_lr()
        self.ckp.write_log('[Epoch {}/{}]\tLearning rate1 : {:.2e}  Learning rate2 : {:.2e}'.format(epoch, self.epoch, Decimal(lr),Decimal(lr2)))
        self.loss_start_log()

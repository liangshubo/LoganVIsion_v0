...
class Trainer(Trainer_1model_1loss):
    ...
    def summarywrite(self):
        ...
        summary_save_path = "/ultrasound/tensorboard_log/liangshubo/"
    ...
    # forward need a idx 
    def model_forward(self,image,idx):
        output = self.model(image,idx)
        return output

    def model_backward(self,batch,image,idx, ground_truth):
        self.optimizer_zero_grad()
        
        output = self.model_forward(image,idx)
    ...
    def model_backward2(self,batch,image, ground_truth_cv1,ground_truth_cv3,ground_truth_cv5):
        self.optimizer_zero_grad()
        loss =0
        for idx in [1.,3.,5.]:
            output = self.model_forward(image,idx)
            output = self.output_process(output)
            if idx ==1:
                loss += self.cala_loss(output, ground_truth_cv1)
            elif idx ==3 :
                loss += self.cala_loss(output, ground_truth_cv3)
            elif idx == 5 :
                loss += self.cala_loss(output, ground_truth_cv5)
    ...
    def train(self):
        ...
        # cv1 cv3 cv5  in controldata   mody by liang in 1212 
        for batch, (image, ground_truth_cv1, ground_truth_cv3, ground_truth_cv5) in enumerate(self.loader_train):
            # - - - - - - - - - - - data_process - - - - - - - - - -
            image, ground_truth_cv1, ground_truth_cv3, ground_truth_cv5 = self.prepare(image, ground_truth_cv1, ground_truth_cv3, ground_truth_cv5)
            if self.down_scale != 1:
                image, ground_truth_cv1, ground_truth_cv3, ground_truth_cv5 = self.down_sample(image,ground_truth_cv1, ground_truth_cv3, ground_truth_cv5)
            # - - - - - - - - - - - start time - - - - - - - - - - -
            timer_data.hold()
            timer_model.tic()
            # - - - - - - - - - - -model forward and loss_optimizer backward three back 
            # for idx in [1,3,5]:
            #     if idx == 1:
            #         self.model_backward(batch,image,idx,ground_truth_cv1)
            #     elif idx == 3 : 
            #         self.model_backward(batch,image,idx,ground_truth_cv3)
            #     elif idx ==5 : 
            #         self.model_backward(batch,image,idx,ground_truth_cv5)
            
            self.model_backward2(batch,image,ground_truth_cv1,ground_truth_cv3,ground_truth_cv5)
    ...
    def test(self):
        ...
        for idx_data, d in enumerate(self.loader_test):
            ...
            # 判断等级
            if d.dataset.name.find('1')>=0:
                idx = 1 
            elif d.dataset.name.find('3')>=0:
                idx = 3
            elif d.dataset.name.find('5')>=0:
                idx = 5
            else:
                idx = int(self.args.test_idx)
            ...
            # tensorboard 
            self.tensorboard_add_scalar("PSNR/Test_{}".format(idx_data),self.ckp.log[-1, idx_data],self.get_epoch())
            self.writer.flush()

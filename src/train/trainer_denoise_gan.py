...
from .base_trainer_gan import Trainer_gan
...
class Trainer_2model(Trainer_gan):
    def __init__(self,args,loader,model_list,loss_list,ckp):
        super(Trainer_2model,self).__init__(args,loader,model_list,loss_list,ckp)
        # self.summarywrite()
        self.loss_gan = nn.BCELoss().cuda()
    def summarywrite(self):
        _,name = self.ckp.dir.split("/experiment/")
        summary_save_path = "/home/ubuntu4090/4T_disk/liangshubo/Sci_Plus/tensor/" # you need set the path 
    ...
    # two
    def cala_loss_g(self, output, mask,fake_g_pred):
        loss1 = self.loss(output, mask)
       
        loss_g  = self.loss_gan(fake_g_pred,torch.ones_like(fake_g_pred).cuda())*0.2 
        
        return loss1+loss_g

    def cala_loss_d(self, output, mask):
        #print(output)
        #print(mask)
        loss_g  = self.loss2(output,mask)
    ...
    def modelg_forward(self,image):
        output1 = self.model(image)
        return output1

    def modeld_forward(self,image):
    ...
    def model_backward(self,image, ground_truth):
        ...
        outputG = self.modelg_forward(image)
        outputD = self.modeld_forward(outputG)
        lossg = self.cala_loss_g(outputG, ground_truth,outputD)
        lossg.backward()
        ...
        # d_back real   --- -- -
        self.model_d_unfreeze()
        self.optimizer2_zero_grad()
        outputD_r = self.modeld_forward(ground_truth)

        loss_r = self.cala_loss_d(outputD_r,torch.ones_like(outputD_r).cuda())
        loss_r.backward()
        outputD_f = self.modeld_forward(outputG.detach().clone())
        loss_f =self.cala_loss_d(outputD_f,torch.zeros_like(outputD_f).cuda())
        loss_f.backward()
        
        
        if self.args.gclip > 0:
            utils.clip_grad_value_(self.model2.parameters(),self.args.gclip)
        self.optimizer2_step()


    def model_d_freeze(self):
        for p in self.model2.parameters():
            p.requires_grad = False
    def model_d_unfreeze(self):
        for p in self.model2.parameters():
    ...
    def train(self):
        ...
        for batch, (image, ground_truth) in enumerate(self.loader_train):
            ...
            self.model_backward(image,ground_truth)
            timer_model.hold()
    ...
    def test(self):
        ...
        for idx_data, d in enumerate(self.loader_test):
            ...
            for image, ground_truth, nameext in tqdm(d, ncols=80):
                ...
                output = self.modelg_forward(image)
    ...
    def eval(self):
        ...
        for idx_data, d in enumerate(self.loader_test):
            ...
            for image, ground_true, nameext in tqdm(d, ncols=80):
                ...
                t_start = time.time()
                output = self.modelg_forward(image_d)
                output = self.output_process(output)

...
class Trainer(Trainer_1model_1loss):
    ...
    def test(self):
        ...
        for idx_data, d in enumerate(self.loader_test):
            ...
            for image, ground_truth, nameext in tqdm(d, ncols=80):
                ...
                DEFINE_IQA += self.output_quality_test(idx_data,output,ground_truth)
            total_forward_time += single_forward_time
            self.ckp.log[-1, idx_data] /= len(d)
            DEFINE_IQA = DEFINE_IQA/len(d)
            best = self.ckp.log.max(0)
            self.test_ckp_logwrite(d,idx_data,best,DEFINE_IQA)
    ...
    def eval(self):
        ...
        for idx_data, d in enumerate(self.loader_test):
            ...
            #f = open(os.path.join('..', 'experiment', self.args.pre_train, "Evluation","results-"+d.dataset.test_dataset_name,"iqa_log.txt"))
    ...
    def output_quality_test(self,idx_data,output,ground_truth):
        self.ckp.log[-1, idx_data] += utility.calc_dice(output, ground_truth)
        softmiou = utility.calc_softmiou(output, ground_truth)
        return softmiou
    def output_quality_eval(self,idx_data,output,ground_truth):
        dice = utility.calc_dice(output, ground_truth)
        self.ckp.log[-1, idx_data] += dice
        mse = utility.calc_skit_mse(output, ground_truth)
        softmiou = utility.calc_softmiou(output, ground_truth)
        return dice,mse,softmiou
    ...
    def test_ckp_logwrite(self,d,idx_data,best,DEFINE_IQA):
        self.ckp.write_log(
            '[{}]\t Dice: {:.3f} Softmiou : {:.3f}(Best: {:.3f} @epoch {})'.format(
    ...
    def evla_ckp_logwrite(self,d,idx_data,MSE,SSIM):
        best = self.ckp.log.max(0)
        self.ckp.write_log(
            '[{}]  Dice: {:.4f} MSE-e3:{:.4f} softmiou:{:.4f}(BestPSNR: {:.4f} @epoch {})'.format(
                d.dataset.test_dataset_name,

...
def make_video(array_list_list,name_list,save_path):
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    print("Save and Cat image ...")
    
    for i in range(len(array_list_list[0])): # 获取每一个列表中的图像索引
        
        # 指定输出视频文件名
        output_video_path = os.path.join(save_path,name_list[i]+".gif")
        h,w = array_list_list[0][i].shape
        #创建视频对象
        
        gif_list = []
        # 组帧写入 
        for j in range(len(array_list_list)):
            
            gif_list.append(array_list_list[j][i])
            # print(f"add image {name_list[i]}")
        # 释放资源 
        #cv2.destroyAllWindows()
        imageio.mimsave(output_video_path, gif_list, duration=1)
...
def mutil_cat(raw_path,folder_list,image_name_list,label_name_listv1,label_name_listv3,label_name_listv5,save_path):
    ...
    make_video(array_list_list,image_name_list,save_path)
...
if __name__ == '__main__':
    ...
    # "Noise","NTEX10+NBL13_B7_E150","NTEX10+NBL13_B4_E15","NTEX10+NBL13_B8_E15","NTEX10+NBL13_B4_E30","NTEX10+NBL13_B3_E60","NTEX10+NBL13_B3_E105"
    ...
    save_path =  os.path.join(raw_path,"Compare_Cat_Videoblur")   #r"E:\Denoise\ModelOutput\Compare_and_cat_N20abdo_cv1\Compare_Cat"

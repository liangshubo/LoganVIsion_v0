...
class ThreeinputBenchmark(BaseDataset):
    def __init__(self,args,test_dataset_name=None):
        super(ThreeinputBenchmark,self).__init__(args,test_dataset_name)
    ...
    def __getitem__(self, idx):
        ...
        tensor_image, tensor_gt,tensor_mask ,namext = self.load_file(image, gt,mask)
        return tensor_image, tensor_gt,tensor_mask,namext
    def crop_image(self,img):
        if self.test_dataset_name.find('N20ABDOcv') >= 0 or self.test_dataset_name.find('N20RENAcv') >= 0 : # 指定数据集来选择不同的裁剪范围与尺寸 
    ...
    def read_file(self,image,gt,mask):
        ...
        if self.test_dataset_name.find('N20ABDOcv')>=0 or self.test_dataset_name.find('N20RENAcv')>=0:

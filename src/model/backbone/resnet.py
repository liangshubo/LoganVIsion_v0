...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)   # h /4   channel = pplane  * exp
        #print(x.shape, "layer1")
        x = self.layer2(x)    # h / 8   channel = pplane  * exp
        #print(x.shape, "layer2")
        x = self.layer3(x)     # h / 16.  channel = pplane  * exp
        #print(x.shape, "layer3")
        x = self.layer4(x)     # h / 32   channel = pplane  * exp
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained: #这里如果要改成我们自己的与训练的效果，需要修改相关的代码了
        #state_dict = load_state_dict_from_url(model_urls[arch],
        #                                      progress=progress)
        path = "/home/ubuntu4090/4T_disk/liangshubo/MSKAI/PreExp/experiment/[ResNet50-n6000-EX6]-[n6000_shoulder_class_all418_v2]-[2025-08-06-10-49]/model/model_best.pt"
        print("Will Load Pretrain model from ",path)
...
def resnet50(pretrained=False, progress=True, **kwargs):
    ...
    return _resnet('resnet50', AttentionBottleneck, [3, 4, 6, 3], pretrained, progress,
                   **kwargs)
...
def resnet101(pretrained=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet101', AttentionBottleneck, [3, 4, 23, 3], pretrained, progress,
                   **kwargs)
...
if __name__ == '__main__':
    model  = resnet50(pretrained=False,num_classes=11, progress=True, replace_stride_with_dilation = [False, True, True])

...
class IC_A2(nn.Module):
    def __init__(self,low_channel,high_channel):
        super().__init__()
        group = high_channel//low_channel
        self.high_channel_reduce = nn.Conv2d(high_channel,low_channel,1)
        self.sig = nn.Sigmoid()
        self.res_block1 = self.resblock(low_channel)
        self.res_block2 = self.resblock(high_channel)
        self.low_channel_reduce = nn.Conv2d(low_channel,low_channel//group,1)

        self.conv1 = nn.Conv2d(2, 1,1, bias=False)
        self.lweight = Parameter(torch.zeros(1, low_channel, 1, 1))
        self.hweight = Parameter(torch.zeros(1, high_channel, 1, 1))


    def resblock(self,in_channel):
        block = nn.Sequential(nn.Conv2d(in_channel,in_channel,3,1,1,groups=in_channel//2),
    ...
    def forward(self,low_feature,high_feature):
        ...
        c2>c1
        '''
        high = self.high_channel_reduce(high_feature)
        attn = self.sig(high)
        f_ca = self.res_block1(low_feature * attn)*self.lweight+low_feature
        out1 = self.low_channel_reduce(f_ca)

        avg_out = torch.mean(out1, dim=1, keepdim=True)
        max_out, _ = torch.max(out1, dim=1, keepdim=True)
        attn = self.conv1(torch.cat([avg_out, max_out], dim=1))

        f_ica = self.res_block2(high_feature * attn)*self.hweight+high_feature
        out = torch.cat([f_ca,f_ica],dim=1)
        return out

class IC_A3(nn.Module):
    def __init__(self,low_channel,middle_channel,high_channel):
        super().__init__()
        group = middle_channel//low_channel
        self.low_channel_reduce = nn.Conv2d(low_channel, low_channel // group,1)
        self.middle_channel_reduce = nn.Conv2d(middle_channel,low_channel,1)
        self.high_channel_reduce = nn.Conv2d(high_channel, middle_channel,1)
        self.sig = nn.Sigmoid()
        self.res_block1 = self.resblock(low_channel)
        self.res_block2 = self.resblock(middle_channel)
        self.res_block3 = self.resblock(high_channel)

        self.conv1 = nn.Conv2d(2, 1,1, bias=False)
        self.conv2 = nn.Conv2d(2, 1, 1, bias=False)

        self.sig = nn.Sigmoid()
        self.convmid2 = nn.Conv2d(middle_channel*2,middle_channel,1)

        self.lweight = Parameter(torch.zeros(1, low_channel, 1, 1))
        self.mweight = Parameter(torch.zeros(1, middle_channel, 1, 1))
        self.hweight = Parameter(torch.zeros(1, high_channel, 1, 1))
    ...
    def forward(self,low_feature,middle_feature,high_feature):
        '''
        low_feature = m,n,c1
        high_feature = m,n,c2
        c2>c1
        '''
        middle = self.middle_channel_reduce(middle_feature)
        attn = self.sig(middle)
        f_ca1 = self.res_block1(low_feature * attn)*self.lweight+low_feature
        out1 = self.low_channel_reduce(f_ca1)

        avg_out = torch.mean(out1, dim=1, keepdim=True)
        max_out, _ = torch.max(out1, dim=1, keepdim=True)
        attn = self.conv1(torch.cat([avg_out, max_out], dim=1))

        f_ica1 = self.res_block2(middle_feature * attn)*self.mweight +middle_feature

        # ----------------------------------------

        high = self.high_channel_reduce(high_feature)
        attn = self.sig(high)
        f_ca2 = self.res_block2(middle_feature * attn)*self.mweight +middle_feature
        out2 = self.middle_channel_reduce(f_ca2)

        avg_out = torch.mean(out2, dim=1, keepdim=True)
        max_out, _ = torch.max(out2, dim=1, keepdim=True)
        attn = self.conv1(torch.cat([avg_out, max_out], dim=1))

        f_ica2 = self.res_block3(high_feature * attn)*self.hweight+high_feature

        f_caic = self.convmid2(torch.cat([f_ica1,f_ca2],dim=1))

        out = torch.cat([f_ca1,f_caic,f_ica2],dim=1)
        return out
...
class DNANet(nn.Module):
    def __init__(self, num_classes, input_channels, block, num_blocks, nb_filter,deep_supervision=False):
        ...
        #
        self.IC0_0_1_0 = IC_A2(nb_filter[0],  nb_filter[1])
        self.IC0_1_1_1 = IC_A2(nb_filter[0], nb_filter[1])
        self.IC0_2_1_2 = IC_A2(nb_filter[0], nb_filter[1])
        self.IC0_3_1_3 = IC_A2(nb_filter[0], nb_filter[1])

        self.IC0_1_1_0_2_0 = IC_A3(nb_filter[0], nb_filter[1],nb_filter[2])
        self.IC0_2_1_1_2_1 = IC_A3(nb_filter[0], nb_filter[1], nb_filter[2])
        self.IC0_3_1_2_2_2 = IC_A3(nb_filter[0], nb_filter[1], nb_filter[2])

        self.IC1_1_2_0_3_0 = IC_A3(nb_filter[1], nb_filter[2], nb_filter[3])
        self.IC1_2_2_1_3_1 = IC_A3(nb_filter[1], nb_filter[2], nb_filter[3])

        self.IC2_1_3_0_4_0 = IC_A3(nb_filter[2], nb_filter[3], nb_filter[4])
    ...
    def forward(self, input):
        ...
        x0_1 = self.conv0_1(self.IC0_0_1_0(x0_0, self.up(x1_0)))

        x2_0 = self.conv2_0(self.pool(x1_0))
        x1_1 = self.conv1_1(self.IC0_1_1_0_2_0(self.down(x0_1),x1_0,self.up(x2_0)))
        x0_2 = self.conv0_2(torch.cat([x0_0, self.IC0_1_1_1(x0_1, self.up(x1_1))], 1))

        x3_0 = self.conv3_0(self.pool(x2_0))
        x2_1 = self.conv2_1(self.IC1_1_2_0_3_0(self.down(x1_1),x2_0, self.up(x3_0)))
        x1_2 = self.conv1_2(torch.cat([x1_0, self.IC0_2_1_1_2_1(self.down(x0_2),x1_1, self.up(x2_1))], 1))
        x0_3 = self.conv0_3(torch.cat([x0_0, x0_1, self.IC0_2_1_2(x0_2, self.up(x1_2))], 1))

        x4_0 = self.conv4_0(self.pool(x3_0))
        x3_1 = self.conv3_1(self.IC2_1_3_0_4_0(self.down(x2_1),x3_0, self.up(x4_0)))
        x2_2 = self.conv2_2(torch.cat([x2_0, self.IC1_2_2_1_3_1(self.down(x1_2),x2_1, self.up(x3_1))], 1))
        x1_3 = self.conv1_3(torch.cat([x1_0, x1_1, self.IC0_3_1_2_2_2(self.down(x0_3),x1_2, self.up(x2_2))], 1))
        x0_4 = self.conv0_4(torch.cat([x0_0, x0_1, x0_2, self.IC0_3_1_3(x0_3, self.up(x1_3))], 1))
...
def make_model(args):
    ...
    nb_filter, num_blocks = load_param('one', 'resnet_18')

    model = DNANet(num_classes=2, input_channels=1, block=Res_DWSA_block, num_blocks=num_blocks,
...
if __name__ == '__main__':
    ...
    x = torch.randn([4, 1, 420, 420]).cuda()
    print(model)

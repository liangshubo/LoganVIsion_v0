...
class NAFNet(nn.Module):
    def __init__(self,inc,ouc,Layer=BaseLayer2,block1=DWConvBlock,block2 = NAFBlock,kersize=3):
        super().__init__()
        nb_filter = [32,64,128,256]
        num_blocks = [1,1,2,8]
        
        self.UNet = UnetModule4_1(input_channels=inc,output_channels=ouc,Layer=Layer,block1=block1,block2= block2,num_blocks=num_blocks,kersize=kersize,nb_filter=nb_filter)
...
def make_model(args):
    model = NAFNet(1,1,Layer=BaseLayer2,block1=DWConvBlock,block2 = NAFBlock,kersize=3)
    return model

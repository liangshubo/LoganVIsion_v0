                ...
                try:
                    ...
                    if name.find('tail') >= 0:
                        ...
                        raise RuntimeError('While copying the parameter named {}, '
                                           ...
                                           'whose dimensions in the ckpoint are {}.'
...
def make_model(args, parent=False):
    if args.sw_mode == 'S':
        return DAVIT(in_chans=1,upscale=1,
                 embed_dim=60,
                 depths=[2,2,2,2,2,2],
                 ...
                 qkv_bias=True,
                 qk_scale=None,
                 train_patchsize = (args.input_size ,args.input_size),
                 norm_layer=nn.LayerNorm,
                 act_layer=nn.ReLU,
                 patch_norm=True,
                 img_range=1.,
        ...
        return DAVIT(in_chans=3,upscale=1,
                 embed_dim=120,
                 depths=[2, 4, 6, 6,4,2],
                 num_heads=[6, 6, 6, 6,6,6],
                 window_size=8,
        ...
        return DAVIT(in_chans=3,upscale=4,
                 embed_dim=180,
                 depths=[6, 6, 6, 6,6,6],
                 num_heads=[6, 6, 6, 6,6,6],
                 window_size=8,
        ...
        return DAVIT(upscale=4, img_size=(32,32),
...
if __name__ == '__main__':
    ...
    # 
    model = DAVIT(in_chans=1,upscale=1,
                 embed_dim=60,
                 depths=[2,4,6,6,4,2],
                 num_heads=[6,6,6,6,6,6],
                 window_size=8,
...
'''
    from option import args
    model = make_model(args)
    dic = torch.load('/home/liangshubo/Image_SR/Code/LSB_Net2/Code/experiment/SWINIR+_X4/model/model_best.pt',map_location='cpu')
    model.load_state_dict(dic)
    lr = '/home/liangshubo/Image_SR/Dataset/benchmark/Thermal_Test/evaluation1/lr_x4_png/'
    path = glob.glob(os.path.join(lr,'*.png'))
    path = sorted(path)
    import cv2
    import torch
    import os
    import torchvision
    for i in path:
        namepath,_ = os.path.split(i)
        name,ext = os.path.splitext(namepath)
        img = cv2.imread(i)
        ten = torch.tensor(img).permute(2,0,1).unsqueeze(dim=0).to(torch.float)
        print(ten.shape)
        out = model(ten)
        print('name {:}'.format(i))
        out = out.squeeze(dim=0).permute(1,2,0).numpy()
        cv2.imwrite('/home/liangshubo/Image_SR/Dataset/benchmark/Thermal_Test/evaluation1/hr/'+_,out)
没有CA
flops : 3.169G
params : 911.319K
